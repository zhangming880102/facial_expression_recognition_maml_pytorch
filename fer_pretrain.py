import  torch, os
import  numpy as np
from    fer2013ForPretrain import FerNShot
import  argparse
import time
from    fer_pretrain_model import Meta
...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        x_spt, y_spt  = torch.from_numpy(x_spt).to(device), torch.from_numpy(y_spt).to(device)

        # set traning=True to update running_mean, running_variance, bn_weights, bn_bias
        accs = maml(x_spt, y_spt)
        ...
        if step % 500 == 0:
            accs = []
            x_qry, y_qry = db_train.next('test')
            x_qry, y_qry = torch.from_numpy(x_qry).to(device), torch.from_numpy(y_qry).to(device)
                # split to single task each time
            test_acc = maml.eval_result(x_qry, y_qry)

            # [b, update_step+1]
            print('Test acc:', test_acc)
            state_dict=maml.state_dict()
            torch.save(state_dict,'models/fer_pretrain_%d.pt'%(step))
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--task_num', type=int, help='meta batch size, namely task num', default=64)
